// mock-cooking.cjs - 简易烹饪工具实现 (CommonJS格式)
...
function execute(input) {
  console.log("执行烹饪工具查询，输入食材:", input.ingredients);
  ...
  if (matchResults.length > 0) {
    return { 
      recipes: matchResults.map(result => `${result.name} (匹配度: ${Math.round(result.matchRate * 100)}%, 找到${result.matchedCount}/${result.totalNeeded}种食材)`)
    };
  } else {
